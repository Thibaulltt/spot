...
PYBIND11_MODULE(spot, spot_module) {
	...
	pybind11::class_<FISTSame>(spot_module, "FISTSamePointClouds")
		...
		.def("get_lap_time", &FISTSame::get_running_time, "lap_number"_a)
		...
		.def_property_readonly("matrix", &FISTSame::get_transform_matrix, pydoc("Returns the computed matrix for the two point clouds, or the identity matrix if not computed."))
		.def_property_readonly("translation", &FISTSame::get_transform_translation, pydoc("Returns the computed translation for the two point clouds, or a null vector if not computed."))
		.def_property_readonly("scaling", &FISTSame::get_transform_scaling, pydoc("Returns the computed scale between the two point clouds, or 1.0 if not computed."))
